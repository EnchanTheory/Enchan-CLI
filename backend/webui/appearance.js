(()=>{
  ...
  function loadBackgroundSettings(){
    try{
      ...
      return {
        darkness:clamp(Number(data.darkness)||55,30,85),
        blur:clamp(Number(data.blur)||2,0,12),
        position:["center","top","bottom","left","right"].includes(data.position)?data.position:"center",
      };
    }catch(_){
      return {darkness:55,blur:2,position:"center"};
    }
  }
  ...
  async function writeStoredImage(blob){
    const db=await openDatabase();
    try{
      await new Promise((resolve,reject)=>{
        ...
        tx.onabort=()=>reject(tx.error||new Error("Image storage was cancelled"));
      });
    }finally{db.close()}
  }
  ...
  async function loadManifest(){
    try{
      ...
      manifest={default:FALLBACK_THEME,themes:[{id:FALLBACK_THEME,name:"Enchan Dark",description:"Original Enchan theme.",swatches:["#1d2021","#2a2b2b","#a59164"]}]};
    }
    renderThemes();
  }

  function setControlsEnabled(enabled){
    for(const control of [darkness,blur,position,removeButton])if(control)control.disabled=!enabled;
  }

  function applyBackground(){
    const enabled=Boolean(backgroundUrl);
    root.classList.toggle("has-custom-background",enabled);
    root.style.setProperty("--background-image",enabled?`url("${backgroundUrl}")`:"none");
    root.style.setProperty("--background-darkness",String(backgroundSettings.darkness/100));
    root.style.setProperty("--background-blur",`${backgroundSettings.blur}px`);
    root.style.setProperty("--background-position",backgroundSettings.position);
    if(preview){
      preview.classList.toggle("has-image",enabled);
      preview.style.backgroundImage=enabled?`linear-gradient(rgba(12,14,15,${backgroundSettings.darkness/100}),rgba(12,14,15,${backgroundSettings.darkness/100})),url("${backgroundUrl}")`:"none";
    }
    if(previewText)previewText.textContent=enabled?"Background image selected.":"No background image selected.";
    if(darkness)darkness.value=String(backgroundSettings.darkness);
    if(darknessValue)darknessValue.value=`${backgroundSettings.darkness}%`;
    if(blur)blur.value=String(backgroundSettings.blur);
    if(blurValue)blurValue.value=`${backgroundSettings.blur}px`;
    if(position)position.value=backgroundSettings.position;
    setControlsEnabled(enabled);
  }

  function replaceBackgroundUrl(blob){
    if(backgroundUrl)URL.revokeObjectURL(backgroundUrl);
    backgroundUrl=blob?URL.createObjectURL(blob):"";
    applyBackground();
  }
  ...
  async function loadBackground(){
    try{
      ...
      if(blob instanceof Blob)replaceBackgroundUrl(blob);
  ...
  imageInput?.addEventListener("change",async event=>{
    ...
    if(file.size>MAX_IMAGE_BYTES){
      error.textContent="Background image must be 20 MB or smaller.";
      event.target.value="";
      return;
    }
    try{
      await writeStoredImage(file);
      replaceBackgroundUrl(file);
    }catch(err){
      error.textContent=err?.message||"Could not store the background image.";
  ...
  removeButton?.addEventListener("click",async()=>{
    ...
    }catch(err){error.textContent=err?.message||"Could not remove the background image."}
  });

  darkness?.addEventListener("input",()=>{
    backgroundSettings.darkness=clamp(Number(darkness.value),30,85);
    saveBackgroundSettings();
    applyBackground();
  });
  blur?.addEventListener("input",()=>{
    backgroundSettings.blur=clamp(Number(blur.value),0,12);
    saveBackgroundSettings();
    applyBackground();
  });
