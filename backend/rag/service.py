...
from backend.rag.search import build_lexical_index, retrieve_candidates, select_candidates
...
class RAGService:
    ...
    def search_all(
        ...
        *,
        ...
        for collection in self.store.list_collections():
            ...
            stale = stale or bool(state["stale"])
            source_missing = source_missing or bool(state["source_missing"])
            ...
            limit = max(1, int(candidate_count or collection.get("candidate_count", 20)))
            limit = min(limit, len(chunks)) if chunks else 1
            found = retrieve_candidates(query, chunks, index, limit)
            for item in found:
                item = dict(item)
            ...
            searched += 1
        if not searched:
            raise RuntimeError("No indexed RAG collections are available. Register a directory first.")
        candidates.sort(key=lambda item: (-float(item.get("score", 0.0)), str(item.get("id", ""))))
        limit = max(1, min(int(selection_count or 6), len(candidates))) if candidates else 1
        combined_index = build_lexical_index(candidates)
        selected, method = select_candidates(query, candidates, limit, index=combined_index, selector=self.selector)
        return {
            "collection": {"id": "all", "name": "All RAG Sources", "source_type": "aggregate"},
            "query": query,
    ...
    def search(
        ...
        *,
        ...
        if reference.strip().lower() in {"all", "*"}:
        ...
        chunks = self.store.load_chunks(collection["id"])
        index = self.store.load_json(collection["id"], "lexical_index.json", {})
        candidate_limit = max(1, int(candidate_count or collection.get("candidate_count", 20)))
        candidate_limit = min(candidate_limit, len(chunks)) if chunks else 1
